    ...
    <div>
      ...
      <div className="border rounded-md p-4 max-h-56 overflow-y-auto space-y-2 bg-gray-50">
        {batches.length <= 0 ? (
          ...
          batches.map((batch) => (
            <label key={batch.code} className="flex items-center gap-3 text-sm cursor-pointer">
              <input
                type="radio"
                name={name}
                value={batch.code}
              ...
              <span>
                {batch.year}
                <span className="text-gray-500 ml-1">({batch.code})</span>
              </span>
            </label>
          ))
        )}
...
export default CheckList_for_ACADEMIC;
