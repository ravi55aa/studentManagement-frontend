...
const RegisterFormBody = () => {
  ...
  const handleValidatePayload = (): boolean => {
      ...
      for (const [key, val] of Object.entries(errorObj)) {
        if (typeof val === 'object' && val !== null && '_errors' in val) {
          ...
          if (key == 'confirmPassword') {
            document.getElementById('reEnter1').textContent =
              errorMessage ?? 'Dear Admin; password should match';
          }
          const errorElement = document.getElementById(key + '1');
  ...
  return (
    <form onSubmit={handleSubmit}>
      <div className="flex flex-col gap-4">
        ...
        {/* Profile Pic Upload */}
        <label className="text-left text-gray-600 text-sm">Profile Picture</label>
        <input
          type="file"
          name="profile"
          onChange={handleInputChange}
