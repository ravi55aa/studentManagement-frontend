...
import { fields, inputStyle } from '@/constants/createSchool';
import { schoolMetaDataValidate } from '@/validation/school.validator';
...
function CreateSchool() {
  ...
  const handleOnSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const isValidated = handleValidationOF(schoolMetaDataValidate, formData);
      ...
      <div className="text-center w-full max-w-md px-4">
        ...
        <form onSubmit={handleOnSubmit}>
          <div className="flex flex-col gap-4">
            {fields.map((ele, i) => {
              return (
                <div key={ele.name + i} className="text-start">
                  <input
                    name={ele.name}
                    type={ele.type}
                  ...
                  />
                  <span id={ele.name} className="text-red-500 errorDisplay  ps-4"></span>
                </div>
              );
            })}

            <label className="text-left text-gray-600 text-sm">School logo</label>
            <input
              type="file"
              name="profile"
